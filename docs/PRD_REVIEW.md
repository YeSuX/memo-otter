...
Memo Otter 的产品目标清楚：做一个由独立开发者自己拥有、可通过 MCP 被 AI 工具调用的个人记忆服务。问题定义也成立，尤其是“跨 AI 工具复用项目上下文、个人偏好和技术决策”这个场景足够具体。

主要风险是 MVP 范围偏大。现有 PRD 同时包含完整 CRUD、语义搜索、状态体系、冲突处理、7 个 MCP 工具、Web UI、统计、JSON export/import、事件日志和部署闭环。对第一版来说，这会让开发重心从“验证记忆服务是否真的有用”转向“搭一个过早完整的平台”。
...
- 归档 memory。
- 导出 JSON。
- MCP 暴露 `save_memory`、`search_memory`、`get_project_context`。
...
- 复杂统计。
- JSON import。
- `deprecate_memory` 和 `delete_memory` 作为 MCP 工具。
...
PRD 已经写了非目标，但 MVP 功能列表仍然容易把产品推向“完整记忆管理后台”。需要在 PRD 里明确第一版验收以 MCP 可用为核心，而不是以 Web UI 完整度为核心。
...
Cloudflare Workers、D1、Vectorize、Workers AI 和 MCP 的组合符合产品目标，也符合学习型项目定位。D1 作为源数据库、Vectorize 作为可重建索引的原则是正确的。
...
- MCP endpoint 采用哪种传输方式和 SDK 版本。
- Workers AI embedding 模型和 Vectorize 维度必须匹配。
- 单一 bearer token 如何同时保护 REST API、Web UI 和 MCP endpoint。
...
- 搜索能召回刚保存的内容。
- MCP 客户端能调用 `save_memory` 和 `search_memory`。
- 未认证请求被拒绝。
- 归档后的 memory 默认不参与搜索。
- 导出的 JSON 可以被人读懂。
...
- MCP 手动验收：在至少一个真实 MCP 客户端中完成保存和搜索。
- 部署冒烟测试：远端保存一条 memory 并搜索回来。

## 6. 评审决策

- MVP 以 MCP 可用和记忆闭环为中心，Web UI 只做基础管理。
- 第一版 MCP 工具缩减为 `save_memory`、`search_memory`、`get_project_context`。
- 第一版使用 Workers AI 作为唯一 embedding provider，只预留接口边界。
- 第一版 UI 和 MCP Server 放在同一个 Worker。
